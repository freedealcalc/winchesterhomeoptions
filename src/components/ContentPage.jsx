...
async function submitLead(formData, placement, slug) {
  ...
  const { error } = await supabase.from('leads').insert({
    ...
    source:      'winchesterhomeoptions.com',
    source_page: slug,
    placement,
...
function MiniForm({ placement, slug, dark = false }) {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <button type="submit" disabled={status === 'loading'} style={{ width: '100%', background: '#B83A3A', color: '#ffffff', border: 'none', padding: '13px 20px', borderRadius: 6, fontFamily: 'var(--font-source-serif), Georgia, serif', fontSize: '0.95rem', fontWeight: 600, cursor: 'pointer' }}>
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        ...
        <div><label style={lbl}>Best Phone *</label><input type="tel" name="phone" value={form.phone} onChange={handleChange} required placeholder="(571) 555-0100" style={inp} /></div>
      </div>
      <div style={{ marginBottom: 16 }}><label style={lbl}>Property Address *</label><input type="text" name="address" value={form.address} onChange={handleChange} required placeholder="123 Main St, Winchester, VA" style={inp} /></div>
...
export default function ContentPage({ config, stats, lastCronRun }) {
  ...
  const trust = trustItems || ['20+ years fixing & flipping in Frederick County', 'Licensed VA real estate agent', 'Pearson Smith Realty', 'Any situation — no judgment'];

  const schema = {
    '@context': 'https://schema.org',
    '@type': type === 'blog' ? 'Article' : 'WebPage',
    name: config.metaTitle, description: config.metaDesc,
    url: `https://www.winchesterhomeoptions.com/${slug}`,
  ...
  const options = [
    ...
    { icon: '🏠', title: 'As-Is MLS Listing', tag: 'Often highest price', tagColor: '#1B2B4B', desc: 'List on the open market as-is — no repairs, no staging. In competitive Frederick County markets, as-is listings attract multiple offers. Often produces more than a single cash offer.' },
    { icon: '🔨', title: 'Wholetail Sale', tag: 'Best of both worlds', tagColor: '#B83A3A', desc: 'Light cleanup, priced right, sold fast — without a full renovation. Faster than traditional, higher than a deep-discount cash offer.' },
  ...
  return (
    <>
      <style>{`
        ...
        .cp-faq-btn:hover { color: #B83A3A !important; }
        .cp-nav-phone:hover { color: #c94f4f !important; }
        .cp-footer-link:hover { color: #c94f4f !important; }
      ...
      {/* NAV */}
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, background: 'rgba(27,43,75,0.97)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(184,58,58,0.2)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 72, maxWidth: 1160, margin: '0 auto', padding: '0 24px' }}>
          <a href="/" style={{ fontFamily: 'var(--font-playfair), Georgia, serif', fontSize: '1.2rem', fontWeight: 700, color: '#ffffff', letterSpacing: '-0.01em', textDecoration: 'none' }}>
            Winchester<span style={{ color: '#B83A3A' }}>Home</span>Options
          </a>
          <div style={{ display: 'flex', alignItems: 'center', gap: 24 }}>
            ...
            <a href="#get-offer" style={{ background: '#B83A3A', color: '#ffffff', padding: '10px 22px', borderRadius: 4, fontFamily: 'var(--font-source-serif), serif', fontWeight: 600, fontSize: '0.9rem', textDecoration: 'none' }}>Get My Offer</a>
          </div>
        </div>
      </nav>
      ...
      <section className="cp-hero-section" style={{ background: '#1B2B4B', color: '#ffffff', padding: '140px 0 80px', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', background: 'radial-gradient(ellipse 80% 60% at 70% 50%, rgba(184,58,58,0.10) 0%, transparent 60%)' }} />
        <div style={{ maxWidth: 1160, margin: '0 auto', padding: '0 24px', position: 'relative', zIndex: 1 }}>
          <div style={{ maxWidth: 700 }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, background: 'rgba(184,58,58,0.10)', border: '1px solid rgba(184,58,58,0.22)', color: '#c94f4f', fontFamily: 'var(--font-dm-mono), monospace', fontSize: '0.72rem', letterSpacing: '0.1em', textTransform: 'uppercase', padding: '8px 16px', borderRadius: 100, marginBottom: 28 }}>
              <span style={{ width: 6, height: 6, background: '#B83A3A', borderRadius: '50%', display: 'inline-block' }} />
      ...
      {/* RED STRIP */}
      <div style={{ background: '#B83A3A', padding: '18px 0' }}>
        <div style={{ maxWidth: 1160, margin: '0 auto', padding: '0 24px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 40, flexWrap: 'wrap' }}>
          {['5 Options — Not Just One Cash Offer', 'Licensed VA Agent · 20+ Years in Frederick County', 'As-Is — No Repairs, No Cleanout', 'Free Consultation · No Pressure'].map((item, i) => (
      ...
      <div style={{ background: '#F7F5F2', padding: '64px 0 0' }}>
        <div style={{ maxWidth: 1160, margin: '0 auto', padding: '0 24px' }}>
          <div style={{ textAlign: 'center', marginBottom: 36 }}>
            <div style={{ fontFamily: 'var(--font-dm-mono), monospace', fontSize: '0.72rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: '#B83A3A', marginBottom: 10 }}>Your options</div>
            <h2 style={{ fontFamily: 'var(--font-playfair), Georgia, serif', fontSize: 'clamp(1.8rem, 3.5vw, 2.6rem)', fontWeight: 700, color: '#1B2B4B', letterSpacing: '-0.02em', marginBottom: 14, lineHeight: 1.2 }}>
              Five Paths Forward — <em style={{ fontStyle: 'italic', color: '#B83A3A' }}>I Know Every One of Them</em>
            </h2>
            <p style={{ fontFamily: 'var(--font-source-serif), Georgia, serif', fontSize: '1.05rem', color: '#5a5a5a', maxWidth: 620, margin: '0 auto', lineHeight: 1.8 }}>
              Most &ldquo;we buy houses&rdquo; companies can offer you one thing — a cash offer, almost certainly below market. As a licensed Virginia agent with 20 years of experience in Frederick County and the Shenandoah Valley, I walk you through every option and tell you honestly which one serves you best.
            </p>
          </div>
          ...
          <div style={{ background: 'linear-gradient(135deg, #1B2B4B, #243860)', borderRadius: 12, padding: '28px 36px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 20, marginBottom: 64 }}>
            ...
            <a href="tel:+15719893269" style={{ display: 'inline-block', background: '#B83A3A', color: '#ffffff', padding: '14px 28px', borderRadius: 6, fontFamily: 'var(--font-source-serif), serif', fontWeight: 600, fontSize: '1rem', textDecoration: 'none', whiteSpace: 'nowrap', flexShrink: 0 }}>
              Call (571) 989-3269
            </a>
          </div>
        </div>
      ...
      <div style={{ background: '#1B2B4B', padding: '56px 0' }}>
        <div style={{ maxWidth: 1160, margin: '0 auto', padding: '0 24px' }}>
          <div style={{ textAlign: 'center', marginBottom: 36 }}>
            <div style={{ fontFamily: 'var(--font-dm-mono), monospace', fontSize: '0.72rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: '#B83A3A', marginBottom: 10 }}>Whatever brought you here</div>
        ...
        <div style={{ maxWidth: 1160, margin: '0 auto', padding: '0 24px' }}>
          <div className="cp-two-col" style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 48, alignItems: 'start' }}>
            ...
            <div className="cp-main-col">
              ...
              {(type === 'city' || type === 'county') && (
                <div style={{ marginBottom: 56 }}>
                  <div style={{ fontFamily: 'var(--font-dm-mono), monospace', fontSize: '0.72rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: '#B83A3A', marginBottom: 10 }}>Live market data</div>
                  ...
                  <div style={{ marginTop: 24, background: 'linear-gradient(135deg, #1B2B4B, #243860)', borderRadius: 12, padding: '36px 32px', color: '#ffffff', position: 'relative', overflow: 'hidden' }}>
                    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 3, background: 'linear-gradient(90deg, #B83A3A, #c94f4f)' }} />
              ...
              {/* How It Works */}
              <div style={{ marginBottom: 56 }}>
                <div style={{ fontFamily: 'var(--font-dm-mono), monospace', fontSize: '0.72rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: '#B83A3A', marginBottom: 10 }}>The process</div>
                <h2 style={{ fontFamily: 'var(--font-playfair), Georgia, serif', fontSize: 'clamp(1.6rem, 3vw, 2.2rem)', fontWeight: 700, color: '#1B2B4B', letterSpacing: '-0.02em', marginBottom: 32 }}>
                  Simple. Fast. <em style={{ fontStyle: 'italic', color: '#B83A3A' }}>On Your Terms.</em>
                </h2>
                <div className="cp-steps-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }}>
                  {[
                    ...
                    <div key={s.n} className="cp-step-card" style={{ background: '#ffffff', borderRadius: 10, padding: '28px 22px', border: '1px solid #e2ddd8' }}>
                      <div style={{ width: 44, height: 44, borderRadius: '50%', background: 'rgba(184,58,58,0.10)', border: '1px solid rgba(184,58,58,0.25)', color: '#B83A3A', fontFamily: 'var(--font-playfair), serif', fontSize: '1.1rem', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 16 }}>{s.n}</div>
              ...
              {/* Reviews */}
              <div style={{ marginBottom: 56 }}>
                <div style={{ fontFamily: 'var(--font-dm-mono), monospace', fontSize: '0.72rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: '#B83A3A', marginBottom: 10 }}>What homeowners say</div>
                <h2 style={{ fontFamily: 'var(--font-playfair), Georgia, serif', fontSize: 'clamp(1.6rem, 3vw, 2.2rem)', fontWeight: 700, color: '#1B2B4B', letterSpacing: '-0.02em', marginBottom: 28 }}>
                  Real Situations. <em style={{ fontStyle: 'italic', color: '#B83A3A' }}>Real Outcomes.</em>
                </h2>
                <div className="cp-reviews-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }}>
                  {[
                    { text: 'Dan explained every option clearly. We did a wholetail and netted $40K more than the cash offer we got elsewhere.', author: 'M.T., Winchester' },
                    { text: 'Inherited a farmhouse in Frederick County and had no idea what to do. Dan walked me through everything with zero pressure. Closed in 3 weeks.', author: 'K.R., Stephens City' },
                    { text: 'Facing foreclosure and thought I had no options. Dan helped me sell fast and kept my credit intact. Called on a Tuesday, closed in 18 days.', author: 'J.P., Front Royal' },
                  ].map((r, i) => (
                    <div key={i} style={{ background: '#ffffff', borderRadius: 10, padding: '28px 24px', border: '1px solid #e2ddd8' }}>
                      <div style={{ color: '#B83A3A', fontSize: '1rem', marginBottom: 10 }}>★★★★★</div>
              ...
              {faqs.length > 0 && (
                <div style={{ marginBottom: 56 }}>
                  <div style={{ fontFamily: 'var(--font-dm-mono), monospace', fontSize: '0.72rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: '#B83A3A', marginBottom: 10 }}>Common questions</div>
                    ...
                    {faqs.map((faq, i) => (
                      <div key={i} style={{ borderBottom: '1px solid #e2ddd8' }}>
                        <button className="cp-faq-btn" onClick={() => setOpenFaq(openFaq === i ? null : i)} style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, padding: '18px 0', fontFamily: 'var(--font-source-serif), Georgia, serif', fontSize: '1rem', fontWeight: 600, color: '#1B2B4B', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left', transition: 'color 0.2s' }}>
                          <span>{faq.q}</span>
                          <span style={{ flexShrink: 0, width: 24, height: 24, borderRadius: '50%', background: '#F7F5F2', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1rem', fontWeight: 700, color: '#B83A3A' }}>{openFaq === i ? '−' : '+'}</span>
                        </button>
              ...
              <div id="get-offer" className="cp-bottom-form" style={{ background: '#ffffff', borderRadius: 12, padding: '40px 36px', border: '1px solid #e2ddd8', boxShadow: '0 4px 24px rgba(27,43,75,0.08)', position: 'relative', overflow: 'hidden' }}>
                <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 4, background: 'linear-gradient(90deg, #B83A3A, #c94f4f)' }} />
                <div style={{ fontFamily: 'var(--font-dm-mono), monospace', fontSize: '0.72rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: '#B83A3A', marginBottom: 10 }}>Free consultation</div>
                ...
                <p style={{ fontFamily: 'var(--font-source-serif), serif', fontSize: '1rem', color: '#5a5a5a', lineHeight: 1.7, marginBottom: 28 }}>
                  No judgment. No obligation. No pressure. Just an honest conversation with someone who has been through it all — and knows {city || county || 'Frederick County'} inside and out.
                </p>
            ...
            <div className="cp-sidebar" style={{ position: 'sticky', top: 88, alignSelf: 'start' }}>
              <div style={{ background: '#ffffff', borderRadius: 12, padding: '28px 24px', border: '1px solid #e2ddd8', borderTop: '4px solid #B83A3A', boxShadow: '0 4px 24px rgba(27,43,75,0.08)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 14 }}>
                  <div style={{ width: 52, height: 52, borderRadius: '50%', background: '#1B2B4B', border: '2px solid #B83A3A', color: '#ffffff', fontSize: '0.9rem', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontFamily: 'var(--font-playfair), serif' }}>DW</div>
                  <div>
                    <div style={{ fontFamily: 'var(--font-playfair), serif', fontWeight: 700, fontSize: '1rem', color: '#1B2B4B' }}>Dan White</div>
                    <div style={{ fontFamily: 'var(--font-dm-mono), monospace', fontSize: '0.7rem', color: '#8a8a8a', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Licensed VA Agent · 20+ Years Frederick County</div>
                  </div>
                </div>
              ...
              <div style={{ marginTop: 16, background: '#ffffff', borderRadius: 12, padding: '20px 24px', border: '1px solid #e2ddd8' }}>
                ...
                {['20+ years fixing & flipping in Frederick County', 'Licensed VA agent — Pearson Smith Realty', 'No seller commission on cash sales', 'Any condition — no repairs, no cleanout', 'Close in 7 days or 90 — your timeline', 'No judgment. No pressure. Ever.'].map((item, i) => (
      ...
      <footer style={{ background: '#0f1c32', padding: '48px 0 32px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ maxWidth: 1160, margin: '0 auto', padding: '0 24px' }}>
          ...
          <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12, fontFamily: 'var(--font-source-serif), serif', fontSize: '0.8rem', color: 'rgba(255,255,255,0.25)' }}>
            <div>© 2026 WinchesterHomeOptions.com · Dan White · Pearson Smith Realty</div>
      ...
      {/* MOBILE BAR */}
      <div className="cp-mobile-bar" style={{ display: 'none', position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 200, background: '#1B2B4B', borderTop: '2px solid #B83A3A', padding: '14px 24px', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 -4px 20px rgba(0,0,0,0.3)' }}>
        ...
        <a href="#get-offer" style={{ padding: '9px 20px', background: '#B83A3A', color: '#ffffff', borderRadius: 4, fontFamily: 'var(--font-source-serif), serif', fontWeight: 600, fontSize: '0.9rem', textDecoration: 'none' }}>Get My Options</a>
      </div>
    </>
  );
}